...
public class ServletUsuario extends HttpServlet {
	...
	private void mostrarFormulario(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			List<Rol> listadoRoles = null;
			UsuarioRolDAO userRolDao = new DAOUsuarioRolImpl();
			listadoRoles = userRolDao.listarRoles();
			request.setAttribute("roles", listadoRoles);
			this.getServletConfig().getServletContext().getRequestDispatcher("/AgregarUsuario.jsp").forward(request, response);
		} catch (Exception e) {
			request.setAttribute("accion", "Login");
			request.setAttribute("Administrador", "administrador");
			this.getServletConfig().getServletContext()
				.getRequestDispatcher("/ServletUsuario").forward(request, response);
		}
	}
